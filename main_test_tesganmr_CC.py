...
def main(json_path):
    ...
    for idx, test_data in enumerate(test_loader):
        ...
        with open(os.path.join(save_dir, 'results.csv'), 'a') as cf:
            writer = csv.writer(cf)
            writer.writerow(['TES-GAN', dataset_opt['mask'], test_results['ssim'][idx], test_results['psnr'][idx]])
    ...
    with open(os.path.join(save_dir, 'results_ave.csv'), 'a') as cf:
        writer = csv.writer(cf)
        writer.writerow(['TES-GAN', dataset_opt['mask'],
                         ave_ssim, std_ssim,
                         ave_psnr, std_psnr,
                         fid])
    # FID ZF
...
if __name__ == '__main__':
    ...
    # Test TES-GAN CCnpi
    main(json_path='options/TESGAN/test/test_tesganmr_CC_G1D30_sample.json')
