...
def main(json_path='options/EESGAN/train_eesganmr_CC_G1D30.json'):
    '''
    # ----------------------------------------
    # Step--1 (prepare opt)
    # ----------------------------------------
    ...
    init_iter_D_g, init_path_D_g = option.find_last_checkpoint(opt['path']['models'], net_type='D_g')
    init_iter_E, init_path_E = option.find_last_checkpoint(opt['path']['models'], net_type='E')
    opt['path']['pretrained_netG'] = init_path_G
    opt['path']['pretrained_netD'] = init_path_D
    opt['path']['pretrained_netD_g'] = init_path_D_g
    ...
    init_iter_optimizerD_g, init_path_optimizerD_g = option.find_last_checkpoint(opt['path']['models'], net_type='optimizerD_g')
    opt['path']['pretrained_optimizerG'] = init_path_optimizerG
    opt['path']['pretrained_optimizerD'] = init_path_optimizerD
    opt['path']['pretrained_optimizerD_g'] = init_path_optimizerD_g
    current_step = max(init_iter_G, init_iter_D, init_iter_D_g, init_iter_E, init_iter_optimizerG, init_iter_optimizerD, init_iter_optimizerD_g)
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_print'] == 0 and opt['rank'] == 0:
                ...
                if 'G_loss_adversarial' in logs.keys():
                    logger_tensorboard.add_scalar('TRAIN Generator LOSS/G_loss_adversarial', logs['G_loss_adversarial'],
                                                  global_step=current_step)
                if 'D_loss_real' in logs.keys():
                    logger_tensorboard.add_scalar('TRAIN Discriminator LOSS/D_loss_real', logs['D_loss_real'],
                                                  global_step=current_step)
                if 'D_loss_fake' in logs.keys():
                    logger_tensorboard.add_scalar('TRAIN Discriminator LOSS/D_loss_fake', logs['D_loss_fake'],
                                                  global_step=current_step)
                if 'D_g_loss_real' in logs.keys():
                    logger_tensorboard.add_scalar('TRAIN Discriminator_g LOSS/D_g_loss_real', logs['D_g_loss_real'],
                                                  global_step=current_step)
                if 'D_g_loss_fake' in logs.keys():
                    logger_tensorboard.add_scalar('TRAIN Discriminator_g LOSS/D_g_loss_fake', logs['D_g_loss_fake'],
                                                  global_step=current_step)
            # -------------------------------
            # 5) save model
            # -------------------------------
...
if __name__ == '__main__':
    ...
    # Train EES-GAN CCnpi
    main('options/EESGAN/train_eesganmr_CC_G1D30.json')