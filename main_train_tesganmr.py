...
TES-GAN (ST-GAN+) for MRI_Recon (EMBC2022)
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_print'] == 0 and opt['rank'] == 0:
                ...
                if 'D_g_loss_real' in logs.keys():
                    logger_tensorboard.add_scalar('TRAIN Discriminator_g LOSS/D_g_loss_real', logs['D_g_loss_real'], global_step=current_step)
                if 'D_g_loss_fake' in logs.keys():
                    logger_tensorboard.add_scalar('TRAIN Discriminator_g LOSS/D_g_loss_fake', logs['D_g_loss_fake'], global_step=current_step)
            # -------------------------------
            # 5) save model
            # -------------------------------
