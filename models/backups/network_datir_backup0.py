...
class RSTB_up(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        x = self.conv(x)
        x = self.patch_embed2(x) + shortcut
...
class DATUNet(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=1,
        ...
        for i_layer in range(self.num_layers // 2):
            if type[i_layer + self.num_layers // 2] == 's':
                ...
                self.decoder.append(layer)
                conv_fix = nn.Conv2d(embed_dims[i_layer + self.num_layers // 2] * 2, embed_dims[i_layer + self.num_layers // 2], 3, 1, 1)
                self.conv_fix_chl.append(conv_fix)
            else:
    ...
    def forward_features(self, x):
        ...
        for idx, layer in enumerate(self.decoder):
            ...
            if idx == 0:
                ...
                x = self.conv_fix_chl[idx](torch.cat([x, x_shortcut[len(self.decoder) - idx - 1]], dim=-1))
                x = layer(x, x_size)
    ...
    def forward(self, x):
        ...
        res = self.conv_after_body(self.forward_features(x_first)) + x_first  # res (batch_size_in_each_GPU, input_image_channel,  H/patch_size, W/patch_size)
