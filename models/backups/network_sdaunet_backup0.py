...
class SwinDAttention(nn.Module):
    ...
    def forward(self, x, window_size, mask=None):
        ...
        if mask is not None:
            attn = attn.view(-1, self.n_head, H * W, H * W)
            nW = mask.shape[0]
            attn = attn.view(B // nW, nW, self.n_head, H * W, H * W) + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.n_head, H * W, H * W)
...
class SwinDATransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        if self.input_resolution == x_size:
            attn_windows, position, reference = self.attn(x_windows, self.window_size,
...
class SDAUNet(nn.Module):
    """ SDAUNet.
    A PyTorch Implement of SDAUNet.
    ...
    def forward_features(self, x):
        ...
        for idx, layer in enumerate(self.decoder):
            ...
            if idx == 0:
                ...
                x = self.unemb_conv_fix_chl[idx](torch.cat([x, x_shortcut[len(self.decoder) - idx - 1]], dim=-1),
                                                 x_size)
...
if __name__ == '__main__':
    ...
    # print('ssddss')
    # model = SDAUNet(img_size=[height, width],
    #                 patch_size = patch_size,
    #                 in_chans=1,
    #                 embed_dim=90,
