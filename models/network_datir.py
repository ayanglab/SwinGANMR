...
class SwinDAttention(nn.Module):
    ...
    def __init__(self, q_size, kv_size, dim, window_size, n_heads, n_groups,
        ...
        self.n_head_channels = self.dim // self.n_heads  # head_dim
        ...
        self.conv_offset = nn.Sequential(
            nn.Conv2d(self.n_group_channels, self.n_group_channels, self.kk, stride, self.kk // 2, groups=self.n_group_channels),
    ...
    def forward(self, x, window_size, mask=None):
        ...
        if mask is not None:
            attn = attn.view(-1, self.n_heads, H * W, H * W)
            nW = mask.shape[0]
            attn = attn.view(B // nW, nW, self.n_heads, H*W, H*W) + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.n_heads, H*W, H*W)
            attn = attn.view(-1, H * W, H * W)
    ...
    def extra_repr(self) -> str:
        return f'dim={self.dim}, num_heads={self.n_heads}, n_groups={self.n_groups}'
    ...
    def flops(self, N):
        ...
        flops += self.n_groups * N * self.n_group_channels * (self.n_group_channels / self.n_group_channels) * self.kk * self.kk
        flops += self.n_groups * N * self.n_group_channels
        flops += self.n_groups * N * self.n_group_channels * 2 * 1
        # self.proj_k(x_sampled)
        flops += N * self.dim * self.dim * 1
        # self.proj_v(x_sampled)
        flops += N * self.dim * self.dim * 1
        # torch.einsum('b c m, b c n -> b m n', q, k)
        flops += self.n_groups * N * self.n_group_channels * N
        # torch.einsum('b m n, b c n -> b c m', attn, v)
        flops += self.n_groups * N * N * self.n_group_channels
...
class SwinDATransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, n_group, window_size=8, shift_size=0,
        ...
        self.attn = SwinDAttention(
            q_size=input_resolution, kv_size=input_resolution,
            dim=dim, window_size=to_2tuple(self.window_size), n_heads=num_heads, n_groups=n_group,
            attn_drop=attn_drop, proj_drop=drop,
            stride=1, offset_range_factor=2,
    ...
    def forward(self, x, x_size):
        ...
        if self.input_resolution == x_size:
            attn_windows, position, reference = self.attn(x_windows, self.window_size, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        else:
            attn_windows, position, reference = self.attn(x_windows, self.window_size, mask=self.calculate_mask(x_size).to(x.device))  # (nW*B, window_size*window_size, C)  nW:number of Windows
...
class SwinDATLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, n_group,
...
class RSDATB_down(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, n_group,
        ...
        super(RSDATB_down, self).__init__()
...
def window_reverse_deform_point(windows, window_size, H, W):
    ...
    for idx_nWH in range(nWH):
        for idx_nWW in range(nWW):
            xx[:, :, idx_nWH * window_size:(idx_nWH + 1) * window_size, idx_nWW * window_size:(idx_nWW + 1) * window_size, 1] \
                = - x[:, :, idx_nWH*window_size:(idx_nWH+1)*window_size, idx_nWW*window_size:(idx_nWW+1)*window_size, 0] \
                  - window_coodinate_2d[idx_nWH, idx_nWW, 0]

            xx[:, :, idx_nWH * window_size:(idx_nWH + 1) * window_size, idx_nWW * window_size:(idx_nWW + 1) * window_size, 0] \
                = x[:, :, idx_nWH * window_size:(idx_nWH + 1) * window_size, idx_nWW * window_size:(idx_nWW + 1) * window_size, 1] \
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=1,
        ...
        for i_layer in range(self.num_layers // 2):
            ...
            if type[i_layer] == 's':
                ...
                layer = RSDATB_down(dim=embed_dims[i_layer],
                                    input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                     patches_resolution[1] // (2 ** i_layer)),
                                    depth=depths[i_layer],
                                    num_heads=num_heads[i_layer],
        ...
        for i_layer in range(self.num_layers // 2):
            ...
            elif type[i_layer + self.num_layers // 2] == 'k':
                layer = RSDATB_up(dim=embed_dims[i_layer + self.num_layers // 2],
                                  input_resolution=(patches_resolution[0] // (2 ** (self.num_layers // 2 - i_layer)),
                                                   patches_resolution[1] // (2 ** (self.num_layers // 2 - i_layer))),
                                  depth=depths[i_layer + self.num_layers // 2],
                                  num_heads=num_heads[i_layer + self.num_layers // 2],
...
if __name__ == '__main__':
    ...
    # print('ssddss')
    # model = DATUNet(img_size=[height, width],
    #                 patch_size=1,
    #                 in_chans=1,
    #                 embed_dim=90,
