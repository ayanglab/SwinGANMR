...
class SwinDAttention(nn.Module):
    ...
    def forward(self, x, window_size, mask=None):
        ...
        return y, pos.reshape(B, self.n_group, Hk, Wk, 2), reference.reshape(B, self.n_group, Hk, Wk, 2), attn.reshape(B, self.n_head, H, W, Hk, Wk)
...
class SwinDATransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        if self.input_resolution == x_size:
            attn_windows, position, reference, attn_map = self.attn(x_windows, self.window_size,
        ...
        return x, position, reference, attn_map  # x (batch_in_each_GPU, H*W, embedding_channel)
    ...
    def forward(self, x, x_size):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                x, pos, ref, att = checkpoint.checkpoint(blk, x, x_size)
                position.append(pos)
                reference.append(ref)
                attn_map.append(att)
            else:
                x, pos, ref, att = blk(x, x_size)
                position.append(pos)
                reference.append(ref)
                attn_map.append(att)
...
class RSDATB_ENCODER(nn.Module):
    ...
    def forward(self, x, x_size):
        assert x.shape[1] == x_size[0] * x_size[1], "input feature has wrong size"
        shortcut = x
        x, position, reference, attn_map = self.residual_group(x, x_size)
...
class SDAUT(nn.Module):
    ...
    def forward(self, x):
        ...
        xx, position, reference, attn_map = self.forward_features(x_first)
